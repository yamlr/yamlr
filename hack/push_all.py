...
REMOTES = {
    ...
    "core": {
        ...
        "exclude": ["src/yamlr/pro"],
        "branch": "main"
    },
    "public": {
        "url": "https://github.com/yamlr/yamlr.git",
        "exclude": ["src/yamlr/pro", "src/yamlr/core", "src/yamlr/parsers", "tests", "tools"],
        "branch": "main"
    }
}
    ...
    try:
        ...
        for path in exclusions:
            if os.path.exists(path):
                # --cached keeps files on disk, but removes from index
                # Wait, if we use --cached, the file stays on disk.
                # When we commit, the deletion is recorded in history.
                # This is what we want. We don't want to delete user's local files.
                run_git(["rm", "-r", "--cached", path], error_ok=True)
