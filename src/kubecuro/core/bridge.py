#!/usr/bin/env python3
"""
AKESO BRIDGE - The Capability Discovery & Identity Layer
-------------------------------------------------------
Enhanced License Management System for Akeso OSS ↔ Kubecuro Pro Integration

Features:
1. Identifies if the Kubecuro Enterprise extension is installed.
2. Validates Pro license status with detailed error messages.
3. Silently ensures both 'akeso' and 'kubecuro' aliases exist (Twin Identity).
...
logger = logging.getLogger("kubecuro.bridge")
console = Console()
...
class ProStatus(Enum):
    """
    Represents the state of Kubecuro Enterprise installation and licensing.
    
    Used to provide detailed feedback to users and enable graceful degradation.
    """
    NOT_INSTALLED = "not_installed"      # akeso.pro package not found
...
# ============================================================================
# AKESO BRIDGE - MAIN INTERFACE CLASS
# ============================================================================

class AkesoBridge:
    """
    Manages the interface between Akeso OSS and Kubecuro Pro.
    ...
    def check_pro_status() -> Tuple[ProStatus, str]:
        ...
        1. Installation Check: Is akeso.pro package present?
        ...
        try:
            ...
            # ----------------------------------------------------------------
            pro_spec = importlib.util.find_spec("akeso.pro")
            
            if pro_spec is None:
                return (
                    ProStatus.NOT_INSTALLED,
                    "Kubecuro Enterprise not installed. Using Akeso OSS features."
                )
            ...
            try:
                ...
                if not hasattr(pro_license, 'validate'):
                    logger.warning(
                        "akeso.pro.license module exists but missing validate() function"
                    )
                    return (
                        ProStatus.INSTALLATION_CORRUPT,
                        "Kubecuro installation incomplete. Reinstall with: pip install --upgrade kubecuro"
                    )
                ...
                return (
                    ProStatus.INSTALLATION_CORRUPT,
                    "Kubecuro installation incomplete. Please reinstall the Enterprise package."
                )
            ...
            try:
                ...
                if "trial" in reason_lower and "expired" in reason_lower:
                    return (
                        ProStatus.TRIAL_EXPIRED,
                        f"{reason}. Activate your license at https://kubecuro.dev/activate"
                    )
                elif "expired" in reason_lower:
                    return (
                        ProStatus.LICENSE_EXPIRED,
                        f"{reason}. Renew at https://kubecuro.dev/renew"
                    )
                elif "invalid" in reason_lower or "corrupt" in reason_lower:
                    return (
                        ProStatus.LICENSE_INVALID,
                        f"{reason}. Contact support at support@kubecuro.dev"
                    )
            ...
            return (
                ProStatus.NOT_INSTALLED,
                "License check unavailable. Using Akeso OSS features only."
            )
    ...
    def is_pro_enabled() -> bool:
        ...
        Usage:
            ...
            ...     run_pro_feature()
        """
        status, _ = AkesoBridge.check_pro_status()
    ...
    def ensure_dual_identity():
        """
        Silently ensures both 'akeso' and 'kubecuro' commands are registered.
        Creates symlinks or copies to enable twin-command invocation.
        
        Behavior:
        - If user runs 'akeso', creates 'kubecuro' alias
        - If user runs 'kubecuro', creates 'akeso' alias
        ...
        try:
            ...
            if "kubecuro" in current_name:
                target_name = f"akeso{suffix}"
            else:
                target_name = f"kubecuro{suffix}"
    ...
    def get_identity() -> str:
        ...
        - "akeso" in name → Always OSS (user preference)
        - "kubecuro" in name → Check if Pro package installed
        - Other names → Default to OSS (prevent false branding)
        
        This prevents wrapper scripts from falsely claiming "PRO" status.
        
        Returns:
            str: 'OSS' if invoked as 'akeso' or Pro not installed,
                 'PRO' if invoked as 'kubecuro' with Pro package present
        
        Examples:
            >>> # User runs: akeso heal deployment.yaml
            >>> get_identity()  # Returns "OSS"
            
            >>> # User runs: kubecuro heal deployment.yaml (with Pro installed)
        ...
        if "akeso" in invoked_as:
            return "OSS"
        
        # Rule 2: Explicit Pro branding request (verify installation)
        if "kubecuro" in invoked_as:
            pro_spec = importlib.util.find_spec("akeso.pro")
            if pro_spec is not None:
                return "PRO"
            else:
                # User typed "kubecuro" but Pro not installed
                logger.warning(
                    "Invoked as 'kubecuro' but Enterprise package not found. "
                    "Install with: pip install kubecuro"
                )
                return "OSS"
    ...
    def get_invoked_command() -> str:
        ...
        Returns:
            str: The exact command name (e.g., 'akeso', 'kubecuro', 'python -m akeso')
        
        Example:
            >>> cmd = AkesoBridge.get_invoked_command()
            >>> print(f"You ran: {cmd}")
        """
    ...
    def notify_pro_required(feature_name: str):
        ...
        Example Output:
            ...
            │                                         │
            │ This feature requires Kubecuro          │
            │ Enterprise extension.                   │
            │                                         │
            │ Visit https://kubecuro.dev to:          │
        ...
        message_parts = [
            ...
            "",
            "This feature requires the [bold]Kubecuro Enterprise[/bold] extension.",
            "",
            "Visit [cyan]https://kubecuro.dev[/cyan] to:",
            "  • Start a 30-day free trial",
            "  • View enterprise pricing and features",
            "  • Request a personalized demo",
            "",
            "[dim]Akeso OSS will continue to provide core healing features.[/dim]"
        ]
    ...
    def notify_license_status(status: ProStatus, message: str):
        ...
        if status == ProStatus.TRIAL_ACTIVE:
            ...
            console.print(Panel(
                ...
                f"{message}\n\n"
                "[cyan]Activate your license at https://kubecuro.dev/activate[/cyan]",
                border_style="red",
                expand=False
            ))
            ...
            console.print(Panel(
                ...
                f"{message}\n\n"
                "[cyan]Renew at https://kubecuro.dev/renew[/cyan]\n"
                "[dim]Or contact sales@kubecuro.dev for assistance[/dim]",
                border_style="yellow",
                expand=False
            ))
            ...
            console.print(Panel(
                ...
                f"{message}\n\n"
                "Please contact [cyan]support@kubecuro.dev[/cyan] with:\n"
                f"  • Your license key\n"
            ...
            console.print(Panel(
                ...
                f"{message}\n\n"
                "Try reinstalling:\n"
                "  [cyan]pip uninstall kubecuro[/cyan]\n"
                "  [cyan]pip install --upgrade kubecuro[/cyan]",
                border_style="red",
                expand=False
            ))
    ...
    def get_pro_module(submodule: str) -> Optional[Any]:
        ...
        try:
            ...
            module = importlib.import_module(f"akeso.pro.{submodule}")
    ...
    def has_pro_license_file() -> bool:
        ...
        from pathlib import Path
        license_file = Path.home() / ".kubecuro" / "license.key"
    ...
    def should_show_daily_reminder() -> bool:
        ...
        reminder_file = Path.home() / ".kubecuro" / "last_reminder.txt"
...
KubecuroBridge = AkesoBridge  # Enterprise naming alias
