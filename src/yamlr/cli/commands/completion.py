...
Register-ArgumentCompleter -Native -CommandName yamlr -ScriptBlock $Script:YamlrCompleter
Register-ArgumentCompleter -Native -CommandName Yamlr -ScriptBlock $Script:YamlrCompleter
Write-Host "Yamlr PowerShell completion loaded." -ForegroundColor Green
"""
...
complete -F _Yamlr_completion yamlr
complete -F _Yamlr_completion Yamlr
"""
...
def handle_completion_command(args, parser: argparse.ArgumentParser, console):
    ...
    if not shell:
         ...
         console.print("  [cyan]Yamlr completion powershell[/cyan]")
         console.print("  [cyan]Yamlr completion bash[/cyan]")
         console.print("  [cyan]Yamlr completion zsh[/cyan]")
         return 1
    ...
    if shell == "powershell":
        ...
        print("\n# Usage: Yamlr completion powershell | Out-String | Invoke-Expression", file=sys.stderr)
    elif shell == "bash":
        print(BASH_TEMPLATE.strip())
        print("\n# Usage: source <(Yamlr completion bash)", file=sys.stderr)
        ...
        print("# 2. Then run: source <(Yamlr completion zsh)", file=sys.stderr)
    else:
