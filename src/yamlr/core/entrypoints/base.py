...
(Moved to Core for Public Safety)
"""
...
# Force UTF-8 and modern Windows terminal support to prevent crashes on exit
console = Console(force_terminal=True, legacy_windows=False, highlight=False)
if platform.system() == "Windows":
    # Rich's auto-detection can sometimes be flaky with capturing outputs
    # Ensuring we don't crash on charmap errors
    import io
...
def print_custom_header(invoked_as: str, is_pro: bool):
    ...
    if invoked_as == "Yamlr":
        ...
        if is_pro:
            ...
            title = "🛡 Yamlr OSS" 
            subtitle = "High-Fidelity Manifest Healing"
            border = "cyan"
    else:
        # Legacy fallback or alias
        title = "🛡 Yamlr OSS"
        subtitle = "High-Fidelity Manifest Healing"
        border = "cyan"
...
def print_version(invoked_as: str, is_pro: bool, cluster_version: str = None):
    ...
    if is_pro:
        badge, license_msg, color = YamlrBridge.get_pro_status_display()
    ...
    if not is_pro:
        tip_text = "[bold]💡 Yamlr Enterprise supports auto-detection from kubectl[/bold]"
    else:
