...
DEFAULT_CONFIG = """# Yamlr Configuration File
# Documentation: https://yamlr.io/config
...
def handle_init_command(console):
    """Generates the .yamlr.yaml file."""
    target = Path(".yamlr.yaml")
    
    if target.exists():
        console.print("[yellow]⚠️  Configuration file .yamlr.yaml already exists.[/yellow]")
        return 1
