#!/usr/bin/env python3
"""
Yamlr FORMATTER - The Visual Heart 
----------------------------------
...
class YamlrFormatter:
    ...
    def print_final_table(self, reports: List[Dict[str, Any]], summary_only: bool = False):
        ...
        if summary_only or len(reports) > 5:
    ...
    def _print_detailed_table(self, reports: List[Dict[str, Any]]):
        ...
        table = Table(
            title="\n[bold magenta]━━━ Yamlr Healing Summary ━━━[/bold magenta]",
            show_header=True,
        ...
        for r in reports:
            ...
            for identity in identities:
                ...
                if findings:
                     ...
                     parts = []
                     if errs: parts.append(f"[red]{errs}E[/red]")
                     if warns: parts.append(f"[yellow]{warns}W[/yellow]")
                ...
                table.add_row(
                    ...
                    issue_str if is_first_row else "",
        ...
        # Summary stats
        console.print(f"\n[bold]Yamlr Engine Summary:[/bold]")
        console.print(f"  Total: {total_files} | "
                     ...
                     f"[dim]○ {unchanged}[/dim] | "
                     f"[red]✗ {failed}[/red]")
        console.print(f"\n[dim]💾 Backups: *.yamlr.backup[/dim]")
        
        # [Smart Tip] If issues exist, guide user to single scan
        has_issues = warnings > 0 or failed > 0 or any(r.get("findings") for r in reports)
        if has_issues:
            console.print(Panel(
                "To see the specific messages for a file listed with issues (e.g., [red]2E[/red]), "
                "you should run a standard scan on that specific file:\n"
                "[bold cyan]Yamlr scan <filename>[/bold cyan]",
                title="💡 SMART TIP",
                border_style="yellow",
                expand=False
            ))
    ...
    def _print_summary_stats(self, reports: List[Dict[str, Any]]):
        ...
        console.print(Panel(
            summary_text,
            title="[bold magenta]━━━ Yamlr Batch Summary ━━━[/bold magenta]",
        ...
        if failed > 0 or by_status.get('WARN', 0) > 0:
             console.print(Panel(
        ...
        console.print(f"[dim]💡 Tip: Detailed table shown automatically for batches <= 5 files.[/dim]\n")
