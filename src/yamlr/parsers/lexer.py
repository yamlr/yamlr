#!/usr/bin/env python3
"""
Yamlr SEMANTIC LEXER (Foundation for Yamlr)
----------------------------------------------
The core sharding engine for yamlr. This module decomposes raw YAML text into 
...
- Extensible: Designed to allow the Yamlr 'Pro' layer to inject custom 
...
logger = logging.getLogger("yamlr.lexer")
...
class YamlrLexer:
    ...
    def _init_heuristics(self):
        """Initializes the heuristic strategy pipeline."""
        from .heuristics.indentation import TabExpansionHeuristic, SnapToGridHeuristic
        from .heuristics.punctuation import MagicColonHeuristic, QuoteBalancingHeuristic, SpacingHeuristic
        
        self.heuristics = [
            TabExpansionHeuristic(),
            SnapToGridHeuristic(),
            MagicColonHeuristic(), # Must run before Spacing to prevent 'image nginx:tag' -> 'image nginx: tag'
            SpacingHeuristic(),
            QuoteBalancingHeuristic()
        ]
    ...
    def repair_line(self, line: str) -> Tuple[int, str, str, bool]:
        """
        Performs surgical line repair using the Heuristic Strategy pattern.
        """
        # 1. Initialize heuristics on first run (lazy load to avoid circular imports)
        if not hasattr(self, 'heuristics'):
            self._init_heuristics()

        # 2. Context for heuristics
        context = {
            'stats': self.repair_stats,
            'in_block': self.in_block
        }
        
        # 3. Apply Heuristic Pipeline
        current_line = line
        
        # Pre-check: Don't heal inside block scalars (mostly)
        # But we need to calculate indent to know if we LEFT the block.
        # This logic is tricky to fully outsource without full context.
        # So we keep the Block State logic here as the "Controller".
        
        raw_indented = current_line.replace("\t", "  ") # Basic safety
        content = raw_indented.lstrip()
        indent = len(raw_indented) - len(content)
        ...
        # 0. Safety Bypass (Ignore Directive) - Case insensitive
        lower_line = line.lower()
        if "# yamlr:ignore" in lower_line or "# ignore" in lower_line:
            # Parse strictly as-is without any healing - preserve ORIGINAL indentation
            raw_ign = line.rstrip()
            original_content = raw_ign.lstrip()
            original_indent = len(raw_ign) - len(original_content)
            
            # Still split comment to separate code from directive
            split_idx = self._find_comment_split(raw_ign)
            if split_idx != -1:
                code_part = raw_ign[:split_idx].strip()
                comment_part = raw_ign[split_idx:].lstrip("# ").rstrip()
            else:
                code_part = raw_ign.strip()
                comment_part = ""
            
            # Maintenance: If ignored line starts a block, track it
            if re.search(r"[|>][+\-]?\d?$", code_part):
                self.in_block = True
                self.block_indent = original_indent + 1
                
            return original_indent, code_part, comment_part, self.in_block

        # Apply Heuristics
        for heuristic in self.heuristics:
            current_line = heuristic.apply(current_line, context)
            
        # Re-calculate indent/content after heuristics might have changed things
        # (e.g. SnapToGrid or Spacing fixes)
        current_line = current_line.replace("\t", "  ") 
        content = current_line.lstrip()
        indent = len(current_line) - len(content)
        
        # Comment extraction
        split_idx = self._find_comment_split(current_line)
        if split_idx != -1:
            code_part = current_line[:split_idx].strip()
            comment_part = current_line[split_idx:].lstrip("# ").rstrip()
        else:
            code_part = current_line.strip()
            comment_part = ""
            
        # Update block state for NEXT line
        if re.search(r"[|>][+\-]?\d?$", code_part):
            self.in_block = True
