#!/usr/bin/env python3
"""
Yamlr SCANNER - The Identity Archeologist
-----------------------------------------
Mines identity markers from Yamlr Shards to extract the "Manifest DNA".
...
from yamlr.core.deprecations import DeprecationChecker
from yamlr.parsers.api_inference import APIVersionInferrer
from yamlr.parsers.schema_manager import SchemaManager
from yamlr.parsers.metadata_extractors import get_extractors

logger = logging.getLogger("yamlr.scanner")
...
class KubeScanner:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            app_name: Name of the application (e.g. "Yamlr") for branding messages
        ...
        self.pro_brand = pro_brand # Emplatix vs Yamlr (Project)
    ...
    def _detect_pro_mode(self) -> bool:
        """
        Detects if Yamlr is running in Pro mode.
        ...
        Detection order:
        1. Environment variable YAMLR_PRO=true
        ...
        # Check environment variable
        if os.getenv("Yamlr_PRO", "").lower() in ("true", "1", "yes"):
            logger.debug("Pro mode enabled via Yamlr_PRO environment variable")
            return True
        
        # Check for license file (future expansion)
        # license_path = Path.home() / ".yamlr" / "license.key"
    ...
    def _extract_cross_resource_metadata(self, shard: Shard, key: str, val: str) -> None:
        ...
        Extractors are located in: yamlr.parsers.metadata_extractors/
    ...
    def _extract_cross_resource_metadata(self, shard: Shard, key: str, val: str) -> None:
        """
        Invokes registered metadata extractors to populate cross-resource fields.
        
        This enables CrossResourceAnalyzer to detect:
        - Ghost Services (selector vs labels matching)
        ...
        Args:
            shard: The current shard being processed
            key: The key from the shard
            val: The cleaned value from the shard
        """
        # Build path keys from the current stack
        path_keys = [pk for (_, pk) in self._path_stack]
        
        # Invoke all registered extractors
        for extractor in get_extractors():
            try:
                extractor.extract_if_applicable(
                    shard=shard,
                ...
                logger.debug(f"Extractor {type(extractor).__name__} failed: {e}")
